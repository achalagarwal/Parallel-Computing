...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks = sites.join(ranks).
                flatMapToPair(kv -> {
                    Double oldRank = kv._2()._2();
                    List<Tuple2<Integer, Double>> contribs = new LinkedList<Tuple2<Integer, Double>>();
                    Website wb = kv._2()._1();
                    Iterator<Integer> iw = wb.edgeIterator();
                    Integer nedges = wb.getNEdges();
                    while(iw.hasNext()){
                        Integer wbs = iw.next();
                        contribs.add(new Tuple2<Integer, Double>(wbs,oldRank/nedges ));

                    }
                    return contribs;
                }).reduceByKey((Double a, Double b)->a+b).mapValues(v->0.15+0.85*v);
        return newRanks;
